...
from classroom.models import Attendances
...
def detect(request):
    ...
    while(True):
        ...
        for(x,y,w,h) in faces:
            ...
            if conf<35:
                userId = getId
                cv2.putText(img, "Recognized",(x,y+h), font, 2, (0,255,0),2)
            else:
        ...
        if(cv2.waitKey(1) == ord('q')):
            ...
            if request.method == 'POST':
                Attendances.objects.filter(student_id=userId,date=current_date).update(statut=1)
                cam.release() 
            cv2.destroyAllWindows()  
            return redirect('/')
    cam.release()
